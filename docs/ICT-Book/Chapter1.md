...
- Micheal believes that the price is controlled and delivered by an algorithm
- The algo is designed to target liquidity, and balancing any price that is imbalance. 
    - Hence why ICT is teachings is base off of liqudity
- Modern Markets are ran by multiple algorithms, not just one master algo. However, all the big players leverages algo to enter and exit the market.
...
*ICT entire concept is based off of understandin the algo, and how the algo fills orders*
- Few common reasons why the *IPA* works:
    1. Liquidity Grab Must Happen Before a Move:
        - algos push prices down to grab sell stops
        - retails sells > instituations buy cheap
        - pump price back up
        - algo knows where liq rest because of the following:
    ...
    3. Why high/lows are swept before major move (Judas Swing)
